...
babyloncc/dist/    committed build output, served directly (.htaccess exception)
...
- Rebuilding the viewer: `cd babyloncc && npm run build`, then commit `dist/`. `dist/compare.html` is hand-written (CDN BabylonJS), not a build output. GLBs in `dist/` need mode 644.
